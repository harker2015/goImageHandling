...
func (imgC *ImageControl) Trimming(before_filename string, after_filename string, x, y, w, h int) {
	src, err := imgC.LoadImage(before_filename)
	if err != nil {
	...
	saveErr := SaveImage(after_filename, img)
	if saveErr != nil {
		log.Println("save image fail..")
	}
}

func (imgC *ImageControl) LoadImage(path string) (img image.Image, err error) {
